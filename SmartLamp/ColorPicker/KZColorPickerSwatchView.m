...
- (id)initWithFrame:(CGRect)frame
{
    ...
    UIImage *stretchableImage = (id)[UIImage imageNamed:@"CheckerBoard.png"];
    self.checkmarkLayer = [CALayer layer];
    self.checkmarkLayer.hidden = YES;
    self.checkmarkLayer.contents = (id)stretchableImage.CGImage;
    //self.checkmarkLayer.contentsScale = [UIScreen mainScreen].scale; //<-needed for the retina display, otherwise our image will not be scaled properly
    //self.checkmarkLayer.contentsCenter = CGRectMake(15.0/stretchableImage.size.width,0.0/stretchableImage.size.height,1.0/stretchableImage.size.width,0.0/stretchableImage.size.height);
    self.checkmarkLayer.bounds = CGRectMake(0, 0, kCheckmarkWidth, kCheckmarkWidth);
    [self.layer addSublayer:self.checkmarkLayer];*/
    
    self.cornerRadius = 16.0f;
    self.hasShadow = YES;
